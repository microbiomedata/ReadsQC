workflow nmdc_rqcfilter {
    String  container="mbabinski17/readsqc:google"
...
task rqcfilter {
     ...
     String gcloud_db_path = "/mounted_workflows_refdata/refdata/RQCFilterData"
    ...
    command<<<
        ...
        if ${gcloud_env}; then
            mkdir -p ${mount_root}
            mkdir -p ${gcloud_db_path}
            gcsfuse --implicit-dirs ${database} ${mount_root}
            if [ -d "/mounted_workflows_refdata/refdata" ]; then
               echo "Directory Exists" 
               ls -l /mounted_workflows_refdata/refdata
            else
